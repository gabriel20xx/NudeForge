...
router.get("/api/carousel-images", (req, res) => {
    ...
    fs.readdir(carouselDir, (err, files) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ error: "Failed to read carousel images" });
        }
        const carouselImages = files.filter((file) =>
...
router.get("/queue-status", (req, res) => {
    ...
    if (requestId) {
        if (requestId === getCurrentlyProcessingRequestId()) {
            ...
            const queueIndex = getProcessingQueue().findIndex(
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const uploadedPathForComfyUI = path.posix.join(
                "input",
                uploadedFilename
            );
            ...
            console.log(
                `[UPLOAD] Received upload: filename=${uploadedFilename}, requestId=${requestId}`
            );
            ...
            console.log(
                `[QUEUE] Added to queue. Queue size: ${
                    getProcessingQueue().length
                }`
            );
...
router.post("/upload-copy", uploadCopy.single("image"), (req, res) => {
    try {
        ...
        res.json({
            message: "Image copy uploaded successfully",
            filename: req.file.filename,
        });
    } catch (err) {
        console.error("[UPLOAD-COPY] Error:", err);
        res.status(500).json({
            error: "Internal server error during upload-copy.",
        });
    }
});
