...
function verifyCaptcha(req, res, next) {
    ...
    if (
        !captcha_answer ||
        !captcha_token ||
        !captchaStore[captcha_token] ||
        captchaStore[captcha_token].toLowerCase() !==
            captcha_answer.trim().toLowerCase()
    ) {
        console.error(
            `CAPTCHA failed or missing. Token: ${captcha_token}, Answer: ${captcha_answer}`
        );
        if (captcha_token) {
